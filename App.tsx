...
import { TRUST_PARTNERS, FAQS, REGIONS } from './constants';
...
const App: React.FC = () => {
  ...
  const nextQuote = () => {
    setActiveQuote((prev) => (prev + 1) % TEAM_QUOTES.length);
  };

  const prevQuote = () => {
    setActiveQuote((prev) => (prev - 1 + TEAM_QUOTES.length) % TEAM_QUOTES.length);
  };
  ...
  return (
    <div className="min-h-screen selection:bg-brand-red selection:text-white flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-brand-red shadow-md h-16 md:h-20">
        <div className="container mx-auto px-4 h-full relative flex justify-between items-center">
          {/* Logo Container - Responsively sized */}
      ...
      {/* Hero Section - Better padding for mobile stacking */}
      <section className="relative pt-12 pb-16 md:pt-32 md:pb-28 overflow-hidden bg-brand-lightGray/30">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 items-start">
            <div className="lg:col-span-7">
              ...
              <div className="mb-8 md:mb-10 max-w-2xl">
                ...
                <div className="space-y-4 text-brand-dark leading-relaxed text-sm md:text-xl">
                  <p>
                    Viele Verwaltungen handeln korrekt – aber nicht immer nachweisbar. Im Schadenfall zählt nicht, was <span className="font-bold">«eigentlich gemacht wurde»</span>, sondern was Sie belegen können. Genau dort entstehen Lücken: Protokolle fehlen, Zyklen sind nicht definiert.
                  </p>
                  <p>
                ...
                {/* Miriam Ganarin Profile Card - Optimized for Mobile UX */}
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 mb-10 md:mb-12">
                <button 
                  onClick={startCheck}
                  className="w-full sm:w-auto px-8 py-4 bg-brand-red text-white text-sm md:text-base font-bold rounded-sm hover:bg-opacity-90 transition-all uppercase tracking-widest shadow-lg active:scale-95"
                >
                  Mit Miriam sprechen
                </button>
                <button 
                  className="w-full sm:w-auto px-8 py-4 border-2 border-brand-dark text-brand-dark text-sm md:text-base font-bold rounded-sm hover:bg-brand-dark hover:text-white transition-all uppercase tracking-widest active:scale-95"
                >
                  Playbook laden
                </button>
              </div>
            ...
            {/* Sticky/Fixed Barometer on Desktop, Block on Mobile */}
            <div id="barometer-anchor" className="lg:col-span-5 w-full">
              {score === null ? (
                <Barometer onFinish={setScore} />
              ) : (
      ...
      {/* Team Slider Section - Fully optimized for Mobile UX */}
      <section className="py-16 md:py-24 bg-white overflow-hidden">
        <div className="container mx-auto px-4">
          ...
          <div className="grid lg:grid-cols-12 gap-10 md:gap-12 items-stretch">
            ...
            {/* Right Column: Quotes Slider - Touch optimized */}
            <div className="lg:col-span-7 flex flex-col justify-between">
              <div className="relative overflow-hidden flex-grow min-h-[360px] sm:min-h-[400px] lg:min-h-0">
                {TEAM_QUOTES.map((item, idx) => (
                  <div 
                    key={idx} 
                    className={`bg-brand-lightGray p-6 sm:p-10 md:p-12 rounded-[16px] absolute inset-0 transition-all duration-700 ease-in-out flex flex-col justify-center ${idx === activeQuote ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12 pointer-events-none'}`}
                  >
                    {/* Decorative Accent */}
              ...
              {/* Slider Controls - Larger tap targets */}
              <div className="flex items-center gap-6 mt-8 justify-center lg:justify-start">
                <button 
                  onClick={prevQuote}
                  className="w-12 h-12 flex items-center justify-center rounded-full border border-brand-lightBlue text-brand-dark hover:bg-brand-red hover:border-brand-red hover:text-white transition-all shadow-sm active:scale-90"
                ...
                <div className="flex gap-3">
                  {TEAM_QUOTES.map((_, idx) => (
                    <button 
                      ...
                      className={`w-3 h-3 rounded-full transition-all duration-300 ${idx === activeQuote ? 'bg-brand-red w-8' : 'bg-brand-lightBlue hover:bg-brand-midBlue'}`}
      ...
      {/* Simplified Problem Context Section */}
      ...
      {/* Solution: 3-Säulen-Modell - Better mobile stacking */}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            <div className="bg-white/5 p-8 border border-white/10 rounded-sm hover:bg-white/[0.08] transition-all group">
              ...
              <p className="text-brand-lightBlue text-sm leading-relaxed opacity-80">Stabilisierung des Objektzustands durch gezielte Prävention an Fassaden and Hotspots.</p>
            </div>
            ...
            <div className="bg-white/5 p-8 border border-white/10 rounded-sm hover:bg-white/[0.08] transition-all group">
              ...
              <p className="text-brand-lightBlue text-sm leading-relaxed opacity-80">Reduktion von Reklamationen durch gepflegte Gebäudehüllen und repräsentative Standards.</p>
            </div>
            ...
            <div className="bg-white/5 p-8 border border-white/10 rounded-sm hover:bg-white/[0.08] transition-all group">
              ...
              <p className="text-brand-lightBlue text-sm leading-relaxed opacity-80">Strukturierte Sorgfaltsnachweise, die intern und gegenüber Eigentümern standhalten.</p>
            </div>
          </div>
        </div>
      </section>

      {/* Process Section - Vertical on mobile, horizontal on desktop */}
      <section className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4 max-w-5xl">
          <h2 className="font-h2 text-center mb-12 md:mb-16 text-brand-dark">Prozess in 3 Schritten</h2>
          <div className="grid md:grid-cols-3 gap-12 md:gap-16">
            {[
              { step: '1', title: 'Assessment', text: 'Wir identifizieren Lücken and analysieren das Bestandsportfolio.' },
      ...
      <section className="py-20 md:py-32 bg-brand-red text-white text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 max-w-3xl relative z-10">
          ...
          <button 
            onClick={startCheck}
            className="w-full sm:w-auto px-10 md:px-14 py-5 md:py-7 bg-white text-brand-red text-base md:text-xl font-bold rounded-sm hover:bg-brand-lightGray transition-all uppercase tracking-widest shadow-2xl active:scale-95"
          >
            Check starten
          </button>
        </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-y-10 gap-x-12 pb-12">
            ...
            <div className="flex flex-wrap items-center gap-x-6 sm:gap-x-10 lg:gap-x-16 gap-y-4 justify-start sm:justify-center">
              {['Zürich', 'Baden', 'Rümlang', 'Basel'].map(loc => (
                <span key={loc} className="font-bold text-xs lg:text-base uppercase tracking-[0.2em] opacity-60 hover:opacity-100 hover:text-brand-red transition-all cursor-default">
                  {loc}
                </span>
              ))}
            </div>

            {/* Direct Contact - Vertical stacking for Mobile */}
